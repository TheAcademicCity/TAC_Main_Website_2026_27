      ...
      <div className="relative z-[1] mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {streams.cards.map((card, index) => (
          <RevealOnScroll key={card.title} delay={Math.min(index, 3) as 0 | 1 | 2 | 3}>
            <article className="h-full border border-white/10 bg-white/6 p-5 transition-[background,transform] duration-300 hover:-translate-y-1 hover:bg-white/10">
              <span className="mb-4 grid h-11 w-11 place-items-center rounded-[10px] bg-white/8">
