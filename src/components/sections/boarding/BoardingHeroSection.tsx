  ...
  return (
    <section className="relative flex min-h-[clamp(320px,48svh,500px)] items-end overflow-hidden bg-forest-deep pt-[var(--site-nav-stack)] md:min-h-[clamp(360px,52vh,500px)]">
      ...
      <Container className="relative z-[2] py-[clamp(1.5rem,4vw,3.5rem)]">
        ...
        <h1 className="max-w-[18ch] font-montserrat text-[clamp(1.7rem,7vw,3.2rem)] font-black leading-[1.06] text-white md:text-[clamp(1.9rem,4.2vw,3.2rem)]">
          {hero.title} <em className="not-italic text-gold">{hero.titleHighlight}</em>
        </h1>
        <p className="mt-3 max-w-[50ch] text-[clamp(0.9rem,3.4vw,1.04rem)] font-light leading-relaxed text-white/70 md:text-[clamp(0.93rem,1.4vw,1.04rem)]">
          {hero.description}
        </p>

        <dl className="mt-5 grid grid-cols-3 gap-x-3 gap-y-4 sm:mt-6 sm:flex sm:flex-wrap sm:gap-9">
          {hero.stats.map((stat) => (
            <div key={stat.label} className="min-w-0">
              <dt className="font-montserrat text-[clamp(1.15rem,4.5vw,1.5rem)] font-black leading-none text-gold sm:text-[1.5rem]">
                {stat.value}
              </dt>
              <dd className="mt-1 text-[0.68rem] font-medium leading-snug text-white/55 sm:text-[0.74rem]">
                {stat.label}
              </dd>
            </div>
          ))}
